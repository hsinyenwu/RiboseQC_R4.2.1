...
options(error=quote({dump.frames(include.GlobalEnv = TRUE)}))
...
for (i in 1:length(rdata_list)){
  ...
  for (j in comps){
    data <- as.data.frame(res_all$selection_cutoffs$results_choice[[j]]$final_choice)
    data_default <- if (nrow(data) == 0) {
      ""
    } else {
      paste0(data$read_length, " (offset=", data$cutoff, ")", collapse = ", ")
    } # read lengths selected by readlength_choice_method, with their P-site offsets
    rl_choices_default <- rbind(rl_choices_default, c(compartment=j, 
                                                      `read lengths of choice (with offset)`=data_default))
  }
